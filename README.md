...
**手勢**：天梯上面左右滑切換上蓋／固鎖／軸心／輔助；面板向下拉關閉（要先捲返到頂）。電腦用左右方向鍵一樣得，Esc 關面板。
...
**同款版本已合併**：同一款上蓋嘅配色、金屬塗層、特別版併成一張卡 —— 279 個上蓋版本收埋成 **138 款**。卡片排喺該款最高嗰個版本嘅等級，但**相用原版嗰個**（例：魔導神杖出 UX-03，唔係金屬塗層版）。撳入去見到每個版本嘅相、型號、各自等級，原版嗰行有「原版」標記，可以逐個標已擁有。

原版點揀：名稱冇後綴嘅優先（「魔導神杖」而唔係「魔導神杖(綠)」或者「金屬塗層:燦金」），`(左)(右)` 當轉向標示唔算後綴；同分就按型號系列 `BX → UX → CX → BXA → BXC → BXG → BXH` 再比數字。
