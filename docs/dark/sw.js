...
var PREFIX  = "beyx-tier-";
var VERSION = "beyx-tier-v12";
var SHELL = [
  "./",
  "./index.html",
  ...
  e.waitUntil(
    caches.keys().then(function(keys){
      return Promise.all(keys.map(function(k){
        return (k.indexOf(PREFIX) === 0 && k !== VERSION) ? caches.delete(k) : null;
      }));
...
self.addEventListener("fetch", function(e){
  ...
  // 其餘：cache 行先，順手背景更新
  e.respondWith(
    caches.open(VERSION).then(function(c){ return c.match(req); }).then(function(hit){
